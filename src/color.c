...
int	pick_color(int iter, t_fractol *fractol)
{
	...
	const double	ratio = (fractol->config.max_iter + 1)
												/ ((double)p.count - 1);
	const double	fract = (iter / ratio) - floor(iter / ratio);
	const int		index = (int)floor(iter / ratio);

	if (fractol->config.p)
		return (interpolate_color(
				p.colors[(index + fractol->config.shift - 1) % (p.count)],
				p.colors[(index + fractol->config.shift) % (p.count)],
				fract));
